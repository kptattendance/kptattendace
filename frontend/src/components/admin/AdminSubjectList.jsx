...
export default function AdminSubjectList({ refreshKey }) {
  ...
  const fetchSubjects = async () => {
    try {
      ...
      // If not admin, restrict to HOD's department
  ...
  useEffect(() => {
    ...
    // Department filter (admins/principal only, HOD already restricted)
    if (deptFilter !== "all") {
      data = data.filter(
        (s) =>
          Array.isArray(s.departments) &&
          s.departments.some(
            (d) => d?.toLowerCase().trim() === deptFilter.toLowerCase().trim()
          )
      );
    }
    ...
    setFiltered(data);
    setPage(1);
  }, [searchTerm, semesterFilter, deptFilter, subjects]);
  ...
  return (
    <div className="rounded-lg p-4 shadow-sm bg-white text-sm">
      ...
      <div className="flex flex-col md:flex-row gap-3 mb-3">
        ...
        {/* Department filter visible only to admins/principal */}
